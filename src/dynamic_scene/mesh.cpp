...
void Mesh::linearBlendSkinning(bool useCapsuleRadius) {
	...
	for (auto v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
		...
		for (auto joint : skeleton->joints) {
			/*Compute the vertex's position with respect to each joint j
			in the skeleton in j's coordinate frame
			when no transformations have been applied to the skeleton 
			(bind pose, vertex bind position)*/
			...
			/*Find where this vertex would end up (in world coordinates)
			if it were transformed along with bone j.*/
			/*Find the closest point on joint j's bone segment (axis)
			and compute the distance to this closest point.*/
			Vector3D closestPos;
			...
			for each j in an LBSInfo structure, 
			which has been defined for you in dynamic_scene/mesh.h*/
			LBSInfo lbsInfo;
		...
		/*Compute the resulting position of the vertex
		by doing a weighted average of the positions from each bone,
		as stored in the LBSInfo structures. 
		The weights for the weighted average should be the inverse distance to the joint,
		so closer bones have a stronger influence.*/
		if (lbsInfos.size() > 0) {
